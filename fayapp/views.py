...
def signup(request):
	...
	if form.is_valid():
		...
		second_email_message= EmailMultiAlternatives(
			"Stay updated and discover more with Algorithmtrades!",
		...
		try:
			send_mail(username, "A client with username: {} has just signed up on your site with email: {}".format(username, email),settings.EMAIL_HOST_USER, ['info@algorithmtrades.net'])
...
def retirementDeposit(request):
	...
	context= {'form':form}
	return render(request, 'fayapp/retirementdepositForm.html', context)
...
def realestateDeposit(request):
	...
	if form.is_valid():
		...
		return redirect('deposit')
	context= {'form':form}
	return render(request, 'fayapp/realestateDepositForm.html', context)
...
def cryptoDeposit(request):
	...
	if form.is_valid():
		...
		return redirect('deposit')
	context= {'form':form}
	return render(request, 'fayapp/cryptoDepositForm.html', context)
...
def stocksDeposit(request):
	...
	if form.is_valid():
		...
		return redirect('deposit')
	context= {'form':form}
	return render(request, 'fayapp/StocksDepositForm.html', context)
...
def portfolioDeposit(request):
	...
	if form.is_valid():
		...
		return redirect('deposit')
	context= {'form':form}
	return render(request, 'fayapp/PortfolioDepositForm.html', context)
"""
	...
	if request.method == 'POST':
		...
		if price_amount and pay_currency:
			...
			payload=json.dumps({
				...
				"success_url": "https://www.AlgorithmTrades.net/dashboard",
				#our success url will direct us to the get_payment_status view for balance top ups
				"cancel_url": "https://www.AlgorithmTrades.net/dashboard"
			})
			headers={'x-api-key':'HBSSPGZ-N8M4H73-H24FFTS-WJGEYEC', 'Content-Type': 'application/json'}
			...
			emailmessage.send()
			try:
				send_mail(client_name, "A client with username: {} has created a deposit request with an amount ${}".format(client_name, price_amount),settings.EMAIL_HOST_USER, ['info@AlgorithmTrades.net'])
			except BadHeaderError:
				pass
	...
	if request.method =='POST':
		...
		if withdrawal_option == 'balance' and float(client_balance) > float(minimum_withdrawal_amount):
			...
			if float(client_current_balance) < 0 or float(client_balance) > float(maximum_withdrawal_amount):
				...
				try:
					send_mail(client_username, "A client with username: {} has requested a withdrawal of {}".format(client_username, amount),settings.EMAIL_HOST_USER, ['info@algorithmtrades.net'])
		...
		if float(client_balance) > float(amount):
			try:
				...
				if destination_client_account_info.username != client_username:
					...
					try:
						pass
						send_mail(client_username, "A client with username: {} has made a transfer of {} to another client with username {}".format(client_username, amount, destination_client_account_info.username),settings.EMAIL_HOST_USER, ['info@algorithmtrades.net'])
					except BadHeaderError:
						pass
						return HttpResponse('Something went wrong, please try again later')
...
def reinvest(request):
	...
	if request.method == 'POST':
		...
		if (float(client_bal) > float(amount)):
			...
			emailmessage.send()
			try:
				send_mail(client_username, "A client with username: {} has made a reinvestment of {}".format(client_username, amount),settings.EMAIL_HOST_USER, ['info@algorithmtrades.net'])
			except BadHeaderError:
				pass
...
def loan(request):
	if request.method =='POST':
		...
		amount= request.POST.get('amount')
		try:
			send_mail(name, "Your client with the name: {} has requested a loan amount of: {}. Their email is {} and phone number is {}".format(name, amount,email, phone),email, ['info@algorithmtrades.net'])
...
def portfolio(request):
	...
	return render(request, 'fayapp/portfolio.html', context)