...
from fayapp.models import Client
	...
	def handle(self, *args, **kwargs):
		...
		for client in Client.objects.annotate(pro= F('deposit') * F('roi') + F('profit'), bal= F('deposit') + F('profit')):
			if float(client.running_days) == float(client.contract_duration):
				...
				template= render_to_string('fayapp/email_top_up_complete.html', {'name':client_username})
				...
				client_username= client.user.username
				client_email= client.user.email
				credit_template= render_to_string('fayapp/email_top_up.html', {'name':client_username})
